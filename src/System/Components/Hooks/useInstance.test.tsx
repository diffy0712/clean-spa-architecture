...
const TestComponent: FC = () => {
    ...
    const instance = useInstance(TestInstance, {});

    return (<>
        <span data-testid="count-span">{count}</span>
        <button data-testid="count-increment" onClick={() => setCount(count + 1)} >+</button>
        <button data-testid="count-decrement" onClick={() => setCount(count - 1)} >-</button>
    </>);
};
...
describe("UseInstance hook test", () => {
    ...
    let constructorMock = jest.spyOn(console, 'log');
    ...
    beforeEach(() => {
        ...
        incrementButton = container.getByTestId('count-increment') as HTMLButtonElement;
        decrementButton = container.getByTestId('count-decrement') as HTMLButtonElement;
    });
