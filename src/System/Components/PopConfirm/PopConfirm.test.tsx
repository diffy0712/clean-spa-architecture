...
describe("PopConfirm tests", () => {
	...
	test("Can render with default values (popover as type)", async() => {
		const {queryByTestId, getByTestId} = render(<PopConfirm>test</PopConfirm>);
	...
	test("Can render with popover", async() => {
		const {queryByTestId,getByTestId} = render(<PopConfirm popover>test</PopConfirm>);
		await userEvent.click(getByTestId('popconfirm-wrapper'));
	...
	test("Can render with modal", async () => {
		const {queryByTestId, getByTestId} = render(<PopConfirm modal>test</PopConfirm>);
		await userEvent.click(getByTestId('popconfirm-wrapper'));
	...
	test("Can call onClick method", async () => {
		...
		const {getByTestId} = render(<PopConfirm onClick={onClickMock}>test</PopConfirm>);
	...
	test("Can display children", async () => {
		const onClickMock = jest.fn(() => true);
		const {getByTestId} = render(<PopConfirm onClick={onClickMock}><div data-testid="popconfirm-test">test</div></PopConfirm>);
